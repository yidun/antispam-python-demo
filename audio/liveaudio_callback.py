        ...
        if len(resultArray) == 0:
            ...
            for result in resultArray:
                ...
                if asrStatus == 4:
                    ...
                    segmentArray: list = result["labels"]
                    if action == 0:
                        print("taskId=%s, 结果: 通过, 证据信息如下: %s" % (taskId, segmentArray))
                    elif action == 1 or action == 2:
                        # for segmentItem in segmentArray:
                        #     label: int = segmentItem["label"]
                        #     level: int = segmentItem["level"]
                        print("taskId=%s, 结果: %s，证据信息如下: %s" % (taskId, "不确定" if action == 1 else "不通过", segmentArray))
    else:
