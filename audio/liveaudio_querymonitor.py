...
# -*- coding: utf-8 -*-
"""
易盾直播音频人审操作记录获取接口python示例代码
接口文档: http://dun.163.com/api.html
python版本：python3.7
运行:
    1. 修改 SECRET_ID,SECRET_KEY,BUSINESS_ID 为对应申请到的值
    2. $ python liveaudio_querymonitor.py
"""
...
if __name__ == "__main__":
    ...
    api = LiveAudioQueryMonitorAPIDemo(SECRET_ID, SECRET_KEY, BUSINESS_ID)
    ...
    if code == 200:
        ...
        if status == 0:
            ...
            for monitor in monitors:
                action: int = monitor["action"]
                actionTime: int = monitor["actionTime"]
                spamType: int = monitor["spamType"]
                spamDetail: str = monitor["spamDetail"]
            print("直播人审结果：%s" % monitors)
        elif status == 20:
            print("数据过期")
        elif status == 30:
            print("数据不存在")
