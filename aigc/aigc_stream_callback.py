# -*- coding: utf-8 -*-
"""
易盾反垃圾云服务AIGC流式检测解决方案离线结果获取接口python示例代码
接口文档: http://dun.163.com/api.html
python版本：python3.7
运行:
    1. 修改 SECRET_ID,SECRET_KEY 为对应申请到的值
    2. $ python aigc_stream_callback.py
"""
...
class AigcStreamCallbackAPIDemo(object):
    """AIGC流式检测解决方案离线结果获取接口示例代码"""
...
if __name__ == "__main__":
    ...
    api = AigcStreamCallbackAPIDemo(SECRET_ID, SECRET_KEY)
    ...
    if code == 200:
        ...
        if resultArray is None or len(resultArray) == 0:
            ...
            for resultItem in resultArray:
                sessionTaskId: str = resultItem["sessionTaskId"]
                sessionId: str = resultItem["sessionId"]
                antispam: dict = resultItem["antispam"]
                if antispam is None:
                    print("Can't find antispam Data")
                else:
                    suggestion: str = antispam["suggestion"]
                    label: str = antispam["label"]
                    print("sessionTaskId=%s, sessionId=%s, suggestion=%s, label=%s" % (sessionTaskId, sessionId, suggestion, label))
    else:
