# -*- coding: utf-8 -*-
"""
易盾反垃圾云服务AIGC流式检测解决方案检测提交接口python示例代码
接口文档: http://dun.163.com/api.html
python版本：python3.7
运行:
    1. 修改 SECRET_ID,SECRET_KEY 为对应申请到的值
    2. $ python aigc_stream_push.py
"""
...
class AigcStreamPushAPIDemo(object):
    """AIGC流式检测解决方案检测提交接口示例代码"""
...
if __name__ == "__main__":
    ...
    api = AigcStreamPushAPIDemo(SECRET_ID, SECRET_KEY)

    # 输入会话检测 demo
    params = {
        "sessionId": "yourSessionId" + str(int(time.time() * 1000)),
        "type": "2",
        "dataId": "yourDataId",
        "content": "当前会话输入的内容",
    ...
    if code == 200:
        ...
        if result is None:
            print("Can't find result Data")
        else:
            sessionTaskId: str = result["sessionTaskId"]
            sessionId: str = result["sessionId"]
            antispam: dict = result["antispam"]
            print("SUCCESS: sessionTaskId=%s, sessionId=%s, antispam=%s" % (sessionTaskId, sessionId, antispam))
    else:
